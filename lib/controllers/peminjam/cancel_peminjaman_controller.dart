...
class CancelPeminjamanController extends GetxController {
  final isLoading = false.obs;
  final pinjamanC = Get.find<PinjamanController>();
  final _provider = Get.find<CancelPeminjamanProvider>();
  final dataCancel = <PeminjamanModel>[].obs;

  @override
  void onInit() {
    super.onInit();
    checkDataWithStatusCancel();
  }

  void checkDataWithStatusCancel() {
    final data = pinjamanC.data
        .where((element) => element.status == 'di_batalkan')
        .toList();

    dataCancel.assignAll(data);
  }

  Future<void> deleteProduct(String id) async {
    final indexData = dataCancel.indexWhere((element) => element.id == id);
    ...
    if (indexData != -1) {
      ...
      dataCancel.removeAt(indexData);

      try {
        await _provider.deletePeminjaman(id);
      } catch (error) {
        Get.snackbar(
          'Error',
          'Error: $error',
